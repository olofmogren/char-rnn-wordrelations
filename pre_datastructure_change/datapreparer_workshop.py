...
def prepare_data(data_dir, id_prob, test_words):
  ...
  if exists:
    for language in supported_languages:
      ...
      #  #print(all_relations[language][relation])
      #  #for partition in relations:
      #  #  print(relations[partition][language][relation])
      #  print('From filesystem: {}: {}: total: {}, num_train: {}, num_valid: {}, num_test: {}'.format(language, relation, len(all_relations[language][relation]), len(relations['train'][language][relation]), len(relations['validation'][language][relation]), len(relations['test'][language][relation])))
      print('{}: {} relations.'.format(language,len(all_relations[language])))
    ...
    print('Total words: train: {}, validation: {}, test: {}'.format(count['train'], count['validation'], count['test']))

    for l in supported_languages:
      toprint = '{}: '.format(l)
      for p in sorted(list(relations)):
        toprint += '[{}: '.format(p)
        ...
        toprint += 'relations: {}, words: {}], '.format(num_rels, num_words)
      print(toprint)
...
def download_data(data_dir, id_prob, test_words):
  ...
  # first_loop: first splitting as in workshop paper. The splitting additional classes
  for language in ['english', 'swedish']:
    num_valid_examples_per_relation = max(4, 600//len(all_relations[language]))
    num_test_examples_per_relation = max(4, 600//len(all_relations[language]))
    relation_labels_fn = os.path.join(data_dir, '{}-relations.txt'.format(language))
    with open(relation_labels_fn, 'w') as f_rel_names:
      for relation in all_relations[language]:
        if first_loop:
          if language in ['english', 'swedish'] and 'pos=V' in relation and not relation.startswith('finite=NFIN,pos=V'):
            continue
        else:
          if language in ['english', 'swedish'] and 'pos=V' in relation and relation.startswith('finite=NFIN,pos=V'):
            continue
          
        print('{}, {}, {}'.format(language, relation, len(all_relations[language][relation])))
        f_rel_names.write(relation+'\n')
        if len(all_relations[language][relation]) > 10:
          random.shuffle(all_relations[language][relation])
        valid_examples = min(num_valid_examples_per_relation, int(0.1*len(all_relations[language][relation])))
        test_examples = min(num_test_examples_per_relation, int(0.1*len(all_relations[language][relation])))
        relations['train'][language][relation]      = all_relations[language][relation][test_examples+valid_examples:]
        relations['validation'][language][relation] = all_relations[language][relation][test_examples:test_examples+valid_examples]
        relations['test'][language][relation]       = all_relations[language][relation][:test_examples]
        for partition in relations:
  ...
  for language in sigmorphon_relations['train']:
    labels = set([])
    for p in sigmorphon_relations:
      labels.update(sigmorphon_relations[p][language])
    relation_labels_fn = os.path.join(data_dir, '{}-relations.txt'.format(language))
    ...
    for p in sigmorphon_relations:
      for relation in sigmorphon_relations[partition][language]:
        ...
        relations[p][language][relation]      = sigmorphon_relations[p][language][relation]
  ...
  # we try to generate identical train-validation-test-split above. (first_loop)
  all_relation_labels_set = set()
  for language in all_relations:
    if language in ['english', 'swedish']:
      relation_names = list(all_relations[language].keys())
      for relation in relation_names:
        if not relation.startswith('finite=NFIN,pos=V'):
          del all_relations[language][relation]
          for partition in relations:
            del relations[partition][language][relation]
        else:
          all_relation_labels_set.update(set(all_relations[language].keys()))
    else:
      all_relation_labels_set.update(set(all_relations[language].keys()))
  all_relation_labels = sorted(list(all_relation_labels_set))
  # each relation is counted twice, as they can be reversed.
  ...
  return prepare_data(data_dir, id_prob, workshop_relation_selection, test_words)
